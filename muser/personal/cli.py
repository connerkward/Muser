...
@app.command()
def train(model: str = typer.Option("siglip2-b", help="Embedding model")):
    """Train the embedding head on user labels (Evaluate tab) and re-bucket the corpus."""
    ensure_personal_root()
    from . import personalness
    r = personalness.train(model=model, progress=lambda m: con.print(f"  {m}"))
    if not r.get("trained"):
        con.print(f"[red]{r.get('message', 'failed')}[/]")
        raise typer.Exit(1)
    con.print(f"\n[bold]supervised training[/] — {r['message']}")
